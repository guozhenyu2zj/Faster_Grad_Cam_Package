...
if os.path.exists(model_path):
    # load csv
    print("csv loading...")
    channel_weight = np.loadtxt(
        os.path.join(model_path, "channel_weight.csv"), delimiter=","
    )
    channel_adress = np.loadtxt(
        os.path.join(model_path, "channel_adress.csv"), delimiter=",", dtype="float"
    )
    ...
    while cap.isOpened():
        ...
        if score < hand_thresh:  # hand is gu
            ...
            heatmap = predict_faster_gradcam(
                channel_out, test_vector, image, kmeans, channel_weight, channel_adress
            )
            if like_OD:
